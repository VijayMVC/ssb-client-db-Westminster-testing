...
GO
	...
	CREATE TRIGGER [dbo].[ExtAttribute10_updated] 
	ON [dbo].[DimCustomer] 
	AFTER UPDATE 
	AS  
	BEGIN 
	IF UPDATE(ExtAttribute10)
		BEGIN 
		UPDATE dbo.DimCustomer 
		SET Nameiscleanstatus = 'dirty' 
		FROM dbo.DimCustomer a 
...
GO
CREATE NONCLUSTERED INDEX [IX_DimCustomer_SSIsDel_SSID] ON [dbo].[DimCustomer] ([SourceSystem], [IsDeleted]) INCLUDE ([SSID])
GO
