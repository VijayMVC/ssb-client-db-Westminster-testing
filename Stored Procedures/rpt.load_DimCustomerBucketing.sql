...
SET ANSI_NULLS ON
GO
CREATE proc [rpt].[load_DimCustomerBucketing]
AS

IF OBJECT_ID('rpt.DimCustomerBucketing', 'U') IS NOT NULL
	DROP TABLE rpt.DimCustomerBucketing 
...
IF OBJECT_ID('tempdb.dbo.#Base','U') IS NOT NULL DROP TABLE #Base
		...
		LEFT JOIN ods.BlackBaud_constituentcodes cc ON a.SSID = cc.[constituent_id]
