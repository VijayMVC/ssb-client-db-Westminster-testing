...
CREATE PROC [dbo].[ValidateCurrentStudents_AdditionalSuiteNo_SS_to_RE] 
	@BizRuleSK	INT 
	AS
...
, MAX(CASE WHEN xx.SourceSystem = 'SeniorSystems' THEN DimCustomerId END)				SS_dimcust
...
, MAX(CASE WHEN xx.SourceSystem = 'SeniorSystems' THEN Gradyear END)				SS_Gradyear
	...
	AND ISNULL(SS_AddressPrimarySuite,'') = ''
